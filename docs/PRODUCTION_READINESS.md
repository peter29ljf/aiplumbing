...
**`eth0` 的 MTU 必须是 1400，不能是 1500。**

到模型端点的路径上有一跳的 MTU 小于 1500，而那条"需要分片"的 ICMP 在路上被过滤掉了。
后果是**小请求正常、大请求永远卡住**：

```
curl 小请求（~200 字节）      HTTP 200   0.74 秒
curl 大请求（28,842 字节）    HTTP 000   120 秒超时
MTU 降到 1400 后，同一个大请求  HTTP 200   1.55 秒
```

agent 的第一次调用带着 27,000 字符的提示和 29 个工具 schema，正好是"大请求"。所以每一轮
都在第一次调用上耗尽全部重试（约 6 分钟），一个工具都没调到，客户看到的是"助理离线"。

**这一层骗了我一整天**：模型能连、小请求 1 秒、`curl` 也说 200——只要请求够小。
`ping -M do` 的探测自相矛盾（1400 通、576 不通），因为 ICMP 本来就是被过滤的那个。

已写进 `/etc/netplan/01-netcfg.yaml`，重启后仍然生效。**不要在没有重新测量的情况下调回 1500。**
判断它是否复发的最快方法：从服务器发一个 28KB 的请求体，正常应该 1–2 秒。
...
那是 MTU 的问题（见开头），不是 agent 慢。修好之后，生产上一轮完整对话：

| 轮 | 耗时 |
|---|---|
| 打招呼 + 描述问题 | 8s |
| 给姓名地址 → 建档 → 查资格 → 查日历 → 报价 | 21s |
| 问有哪些时间 | 5s |
| 选时间 → 建预约 → 发确认短信 | 15s |

同时客户能看到进度：`Looking up your details` → `Checking we can take this on` →
`Checking what the visit costs` → `Booking that in` → `Sending your confirmation`。

原来那节记的东西留档

不是 bug，是 agent 第一轮要做的事就是多：查客户、读规则、看日历、算 call-out 费。实测 **129 秒**。

已经不再是链路问题（`/chat/message` 立刻返回、前端轮询），但对客户来说仍然是**盯着三个点看两分钟**。

能压的方向，按性价比：
1. 首轮少调几个工具——`rules.get_company_info` 之类可以合并进系统提示，不必每次现查
2. 首轮先回一句"我看一下你的记录"，再继续——把感知延迟砍到一次模型调用
3. 换更快的模型（现在 `deepseek-v4-flash-0731`；端点本身不慢，DNS 11ms / TCP 70ms / 小请求 1.4–2.5s）
