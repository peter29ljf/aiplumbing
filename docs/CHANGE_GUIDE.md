...
### 合并前：`bash scripts/gate.sh`

按改动内容分两档，便宜的先跑，第一个失败就停：

| 档 | 跑什么 | 何时 |
|---|---|---|
| **快** | 检查器自检 → 静态扫描 → 单测 | 总是。约 4 秒，零 API |
| **全** | 加上 live 套件 `--repeat 4` | 改动碰到 `agents/`、`config/`、`scenarios/` 时 |

**为什么分档**：让改一个错字也要烧 300 万 token，结果是没人再跑这个门槛。
