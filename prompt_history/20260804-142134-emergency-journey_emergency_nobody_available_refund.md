...
- Time: 2026-08-04T14:21:34.700527
- Backend: claude_cli:claude-opus-5
- Triggering scenario: journey_emergency_nobody_available_refund
- Files: agents/emergency.md
- Reason: The agent read an empty `phone.list_available_technicians` result as "nobody can come" and skipped calling anyone, so `phone.call_technician` was never reached; I added a rule to `agents/emergency.md` Step 4 that an empty candidate list means the filter was too narrow (widen it, use `excluded`) and that the customer may only be told nobody is available after actual calls have been placed and declined or unanswered.

## agents/emergency.md — before
...
**An empty candidate list is not an answer — it means your filter was too narrow.** That tool
only sorts the roster; it does not phone anyone, so on its own it tells you nothing about who
would come. If `available` comes back empty, widen and look again: drop the `skill` argument
first, then drop `area` as well, and read `excluded` to see who was ruled out and why. At this
time of night anyone on duty and under their job limit is worth a call — the technician
decides whether they can take the job, not the filter.

**You may only tell a customer that nobody is available after `phone.call_technician` has
actually been used** and those people declined or did not pick up. Until a call has been
placed, no one has been asked, and saying otherwise to someone who has just paid a deposit is
