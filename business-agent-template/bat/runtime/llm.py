...
class LLM:
    ...
    def __init__(self, cfg: dict[str, Any] | None = None) -> None:
        """`cfg` is a project's model.yaml. Required in substance — an LLM with no
        provider block used to fall back to one company's config file, which is a very
        confusing way to bill the wrong account."""
        load_dotenv()
        self.cfg = cfg or {}
